...
import NewSphrSeqFFTVisPKG.clef.myClef;
import NewSphrSeqFFTVisPKG.clef.myGrandClef;
import NewSphrSeqFFTVisPKG.clef.base.myClefBase;
import NewSphrSeqFFTVisPKG.clef.enums.clefType;
import NewSphrSeqFFTVisPKG.instrument.myInstrument;
import NewSphrSeqFFTVisPKG.note.NoteData;
import NewSphrSeqFFTVisPKG.note.myNote;
import NewSphrSeqFFTVisPKG.note.enums.chordType;
import NewSphrSeqFFTVisPKG.note.enums.noteDurType;
import NewSphrSeqFFTVisPKG.note.enums.noteValType;
import NewSphrSeqFFTVisPKG.score.myScore;
import NewSphrSeqFFTVisPKG.ui.myInstEditWindow;
import NewSphrSeqFFTVisPKG.ui.mySequencerWindow;
import NewSphrSeqFFTVisPKG.ui.mySimWindow;
import NewSphrSeqFFTVisPKG.ui.mySphereWindow;
 ...
 * 
 */
 public class NewSeqVisFFTOcean extends PApplet implements IRenderInterface{
	...
	public static void main(String[] passedArgs) {		
		String[] appletArgs = new String[] { "NewSphrSeqFFTVisPKG.SeqVisFFTOcean" };
			if (passedArgs != null) {
...
//			drums7 : talk drum			
	...
	//	if (cyclModCmp) {															//if drawing this frame, draw results of calculations								
			...
			for(int i =1; i<numDispWins; ++i){if((isShowingWindow(i)) && (dispWinFrames[i].dispFlags[myMusicSimWindow.is3DWin])){dispWinFrames[i].draw(myPoint._add(sceneOriginVals[sceneIDX],focusTar));}}
			popStyle();popMatrix();
...
//		}		
	...
	public void drawUI(float modAmtMillis){					
		for(int i =1; i<numDispWins; ++i){if ((isShowingWindow(i)) && !(dispWinFrames[i].dispFlags[myMusicSimWindow.is3DWin])){dispWinFrames[i].draw(sceneOriginVals[sceneIDX]);}}
	...
	public void setCamView(){//also sets idx in scene focus and center arrays
		sceneIDX = (curFocusWin == -1 || NewSeqVisFFTOcean.dispWinIs3D[curFocusWin]) ? 1 : 0;
...
//				case '"' : {((mySideBarMenu)dispWinFrames[dispMenuIDX]).guiObjs_Numeric[((mySideBarMenu)dispWinFrames[dispMenuIDX]).gIDX_cycModDraw].modVal(1); break;}//increase the number of cycles between each draw to some upper bound		
	...
	public void endShiftKey(){
		...
		for(int i =0; i<NewSeqVisFFTOcean.numDispWins; ++i){dispWinFrames[i].endShiftKey();}
	}
	public void endAltKey(){
		clearFlags(new int []{altKeyPressed});
		for(int i =0; i<NewSeqVisFFTOcean.numDispWins; ++i){dispWinFrames[i].endAltKey();}			
	}
	public void endCntlKey(){
		clearFlags(new int []{cntlKeyPressed});
		for(int i =0; i<NewSeqVisFFTOcean.numDispWins; ++i){dispWinFrames[i].endCntlKey();}			
	}
...
/////////////////////
	...
	@Override
	public void drawRenderBackground(int r, int g, int b, int alpha) {
