package NewSphrSeqFFTVisPKG.ui.controls;
...
//			myVector noteCtr = new myVector(ctrOfNotes);
		...
		if(tmp != null){
			...
			if(!tmp.flags[myNote.isChord]){						//no chord here, make new chord, put in treemap here
				...
				if(!tmp.equals(note)){				//if tmp!=note then make tmp a chord, and add note to temp
					...
					myChord tmpChord = new myChord(tmp.sphereDims[0], tmp.sphereDims[1], tmp.sphereRing, tmp.sphrOwn);					
	...
	private static final float tickScale = 4 * noteDurType.Whole.getVal() * 1.0f/(MyMathUtils.DEG_TO_RAD_F * 360.0f);
	...
	public myNote getNoteFromSphereLoc(myPoint pt){
		...
		myNote res = new myNote(alphaSt, (alphaSt + noteAlphaWidth), clickRing, this);		
