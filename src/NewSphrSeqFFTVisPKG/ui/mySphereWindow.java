...
//	}//addSphereNoteToPlayNow	
	...
	public void drawTunnel(){
		...
		for (int i = 0; i < numTunnelCircles; i++) { 
			...
			a = (int)MyMathUtils.map((float)(tunnelCrcls[i].z), zMin, zMax, 111,255); 
	...
	protected void drawMe(float animTimeMod) {
		...
		//msgObj.dispInfoMessage("mySphereWin","drawMe","Current mouse loc in 3D : " + curMseLoc3D.toStrBrf() + "| scenectrvals : " + pa.sceneOriginVals[pa.sceneIDX].toStrBrf() +"| current look-at vector from mouse point : " + curMseLookVec.toStrBrf());
		pa.pushMatState();
		pa.noLights();
		setLights();
